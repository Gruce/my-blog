...
// Mobile sidebar toggle - start as false
...
// Default to desktop width for SSR (desktop sidebar should render)
// Will be updated on mount for accurate client-side rendering
...
// Computed style for sidebar opacity
  ...
  <template v-if="page">
    ...
    <ClientOnly>
      <div v-if="sidebarOpen" class="fixed inset-0 bg-black/50 z-40 lg:hidden" @click="sidebarOpen = false"
        @touchstart="(e: TouchEvent) => { e.preventDefault(); sidebarOpen = false; }"></div>
    </ClientOnly>
    ...
    <Teleport to="body">
      <aside 
        ...
        <div ref="sidebarScrollRef" class="w-full overflow-y-auto p-4 h-full"
          style="-webkit-overflow-scrolling: touch; touch-action: auto;">
          <!-- Sidebar Header -->
          <div class="mb-6 pb-6 border-b border-zinc-800">
            <NuxtLink to="/" @click="sidebarOpen = false" class="flex items-center gap-3 group">
              ...
              <div class="min-w-0 flex-1">
                <h2 class="text-sm font-medium tracking-tight transition-colors" :class="headerNameClass">Hassan K.
                  Al-Khalidi</h2>
                  ...
                  <template v-for="item in section.items" :key="'seriesName' in item ? item.seriesName : item.id">
                    ...
                    <div v-if="'seriesName' in item">
                      <button @click.stop="toggleSeries(item.seriesName)"
                        ...
                        <span class="text-xs transition-colors" :class="sectionCountClass">({{ item.items.length
                          }})</span>
                      </button>

                      <!-- Series Articles -->
                      <div v-if="isSeriesExpanded(item.seriesName)" class="ml-6 mt-0.5 space-y-0.5">
                        <NuxtLink v-for="post in item.items" :key="post.id" :to="post.path" @click="sidebarOpen = false"
                          class="flex items-center gap-2 py-1.5 pl-1 text-sm rounded-md transition-colors"
                    ...
                    <!-- Standalone Post -->
                    <NuxtLink v-else :to="item.path" @click="sidebarOpen = false"
                      class="flex items-center gap-2 py-1.5 pl-1 text-sm rounded-md transition-colors"
    ...
    <div class="flex" :class="{ 'lg:ml-80': !isClient || windowWidth >= 1024 }">
      ...
      <ClientOnly>
        <Transition name="slide-left">
          <aside v-if="windowWidth < 1024 && sidebarOpen"
            class="lg:hidden fixed left-0 top-0 h-screen w-80 border-r border-zinc-800 bg-zinc-950 z-50" @click.stop>
          <div ref="sidebarScrollRef" class="w-full overflow-y-auto p-4 h-full"
            style="-webkit-overflow-scrolling: touch; touch-action: auto;">
              ...
              <template v-for="section in sections" :key="section.key">
                <div>
                  <button @click="toggleSection(section.key)"
                    ...
                    <span class="flex-1 text-left">{{ section.title }}</span>
                    <span class="text-xs transition-colors" :class="sectionCountClass">({{ section.items.length
                      }})</span>
                  </button>
                  <div v-if="isSectionExpanded(section.key)" class="ml-6 mt-0.5 space-y-0.5">
                    <template v-for="item in section.items" :key="'seriesName' in item ? item.seriesName : item.id">
                      <div v-if="'seriesName' in item">
                        <button @click.stop="toggleSeries(item.seriesName)"
                          ...
                          :class="seriesButtonClass" type="button">
                          <svg class="w-4 h-4 flex-shrink-0 transition-transform duration-200"
                            :class="{ 'rotate-90': isSeriesExpanded(item.seriesName) }" fill="none"
                        ...
                        <div v-if="isSeriesExpanded(item.seriesName)" class="ml-6 mt-0.5 space-y-0.5">
                          <NuxtLink v-for="post in item.items" :key="post.id" :to="post.path"
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="mb-6 flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm text-zinc-500">
              <time v-if="page.date" :datetime="page.date" class="tabular-nums">{{ new
                Date(page.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: '2-digit' })
              }}</time>
